              ...
              <ul class="listBox">
                <li>{{$r->name}}
                  <ul class="listBox">
                    @foreach ($r->users as $u)
                      <li>
                        ...
                        ($r->name === 'Super-Admin' && $canDeleteSuperAdmins)
                          ...
                          <form class="form-inline" role="form" method="POST" action="{{ route('a.r.delete', $r->id) }}">
                            @csrf  @method('delete')
                    ...
                    @endforeach
                      @if($canEdit)
                        <form class="form-horizontal d-print-none" role="form" method="POST" action="{{ route('a.r.add') }}">
                          @csrf  @method('post')
                          <input type="hidden" name="role_id" value="{{ $r->id }}">
